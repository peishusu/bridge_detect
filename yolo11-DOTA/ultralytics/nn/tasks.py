...
def parse_model(d, ch, verbose=True):  # model_dict, input_channels(3)
    ...
    for i, (f, n, m, args) in enumerate(d["backbone"] + d["head"]):  # from, number, module, args
        ...
        if m in {
            ...
            CoordAtt,
            SEAttention
            # todo 如果你希望你的模块是跟随网络的尺度进行变化的，请看这个位置的，其中width作用的就是通道的数量
        }:
            c1, c2 = ch[f], args[0]
            # print(c1, c2)
            ...
            if m is C3k2:  # for M/L/X sizes
                ...
                if scale in "mlx":
                    args[3] = True
        elif m in {ChannelAttention, # todo  这里是只需要提供一个参数的（通过这种方式配置的权重不会随着改变）
            }:
            args = [ch[f], ]
            # args = [*args ]
            # 后面是将其余的参数也就是配置文件里面的参数解析出来
        elif m in {                 # todo  这里是需要直接从配置文件中进行参数解析，不需要添加通道数量的 （通过这种方式配置的权重不会随着改变）
            SpatialAttention}:
            # args = [ch[f], ]
            args = [*args ]
        elif m is AIFI:
